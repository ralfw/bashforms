...
namespace bashforms_tests
{
    ...
    public class MenuItemStack_tests
    {
        ...
        public void Acceptance()
        {
            ...
            var a = items.AddItem(new MenuItem("a", "ia"));
            ...
            var b = items.AddItem(new MenuItem("b", "ib"));
            var b1 = b.Submenu.AddItem("b1");
            b1.Submenu.AddItem("b11");
            b1.Submenu.AddItem("b12");
            b.Submenu.AddItem("b2");
            items.AddItem(new MenuItem("c", "ic"));
            
            var sut = new MenuItemStack(items);
            
            var pathItemTexts = sut.PathMenuItems.Select(i => i.Text).ToArray();
            Assert.AreEqual(0, pathItemTexts.Length);

            var currentItemTexts = sut.CurrentMenuItems.Select(i => i.Text).ToArray();
            Assert.AreEqual(new[]{"a", "b", "c"}, currentItemTexts);

            var item = sut.PushItem(0); // +a
            Assert.AreSame(a, item);
            
            pathItemTexts = sut.PathMenuItems.Select(i => i.Text).ToArray();
            Assert.AreEqual(new[]{"a"}, pathItemTexts);
            
            currentItemTexts = sut.CurrentMenuItems.Select(i => i.Text).ToArray();
